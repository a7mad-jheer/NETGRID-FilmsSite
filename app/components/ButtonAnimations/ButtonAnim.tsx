...
export default function ButtonAnim({ label, onClick, className, disabled = false, icon , link }: ButtonAnimProps) {
    return (
        <>
            <style>{`
                @keyframes shine {
            ...
            <div className="button-bg rounded-full p-0.5 hover:scale-105 transition duration-300 active:scale-100">
                <button className={`lg:px-8 text-sm lg:py-2.5 px-6 py-2 h-full text-white rounded-full font-medium bg-gray-800 ${className || ""}`}
                onClick={onClick}
                disabled={disabled}
                >
                    {disabled ? <Spinner /> : link ? <Link href = {link}>{label}</Link> : label}
                    {icon && icon}
                </button>
            </div>
        </>
